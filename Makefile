...
create-base:
	...
	@{ \
		...
		echo "Running command: spl-token --program-id TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb create-token --enable-metadata --enable-freeze" | tee -a $(LOG_FILE); \
...
initialize-metadata: create-base
	...
	@{ \
		...
		echo "Running command: spl-token initialize-metadata $(TOKEN_ADDRESS) $(TOKEN_NAME) $(TOKEN_SYMBOL) $(TOKEN_URI)"; \
...
create-account: initialize-metadata
	...
	@{ \
		...
		echo "Running command: spl-token create-account $(TOKEN_ADDRESS)"; \
...
mint-tokens: create-account
	...
	@{ \
		...
		echo "Running command: spl-token mint $(TOKEN_ADDRESS) 100000"; \
...
create-init-display: mint-tokens
	...
	@{ \
		...
		echo "Running command: spl-token display $(TOKEN_ADDRESS)"; \
